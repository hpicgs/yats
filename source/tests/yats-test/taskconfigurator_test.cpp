...
{
public:
	const std::map<uint64_t, yats::AbstractInputConnector>& inputs() const { return yats::TaskConfigurator<T>::m_inputs; }
	const std::map<uint64_t, yats::AbstractOutputConnector>& outputs() const { return yats::TaskConfigurator<T>::m_outputs; }
};
